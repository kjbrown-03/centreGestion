  ...
  return (
    <DashboardLayout allow="infirmier" title="Soins du jour">
      <div className="grid sm:grid-cols-4 gap-5">
        <StatCard label="Soins planifiés" value="18" icon={Syringe} accent />
          ...
          <div className="mt-5 space-y-2">
            {tasks.map((t, i) => (
              <motion.div
                ...
                whileHover={{ x: 4 }}
                className={`flex items-center gap-4 p-4 rounded-2xl border transition hover:bg-muted/40 ${t.urgent ? "border-amber-400/50 bg-amber-50/40" : ""}`}
              >
                ...
                <div className="flex-1">
                  <p className="font-semibold text-[color:var(--navy)]">{t.patient}</p>
                  <p className="text-sm text-muted-foreground">{t.act}</p>
                </div>
                {t.urgent && <span className="text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-amber-500 text-white">Urgent</span>}
              </motion.div>
            ))}
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-3xl gradient-hero p-7 text-white">
          ...
          <div className="mt-5 space-y-3">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-3 rounded-xl glass-dark">
                <p className="text-sm">{p.name}</p>
                <p className={`font-mono font-semibold ${p.color}`}>{p.v}</p>
              </div>
            ))}
          </div>
        </motion.div>
